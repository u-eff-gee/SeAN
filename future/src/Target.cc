...
vector<double> Target::energies(const double e_min, const double e_max,
    ...
    for(size_t i = 0; i < nuclei.size(); ++i){
        for(size_t j = 0; j < nuclei[i].get_n_excited_states(); ++j){
            if(nuclei[i].doppler_width(i, temperature) > nuclei[i].get_excited_state(i).get_total_width()){
                ene_state = grid.normal(nuclei[i].get_excited_state(i).get_excitation_energy(),
                    nuclei[i].doppler_width(i, temperature),
                    e_min, e_max, n_energies_per_state);
            } else{
                ene_state = grid.breit_wigner(nuclei[i].get_excited_state(i).get_excitation_energy(),
                    nuclei[i].doppler_width(i, temperature),
                    e_min, e_max, n_energies_per_state);                
            }
