...
\label{absorption_general_factorized}
	W = \left| M^{\mathrm{nucl}}_{C \to B} \right|^2 \cdot \left| M^{\mathrm{nucl}}_{A \to C} \right|^2 \left| \sum_{n_s} \frac{ \langle \beta_s | \exp{\left( - i \mathbf{k} \cdot \mathbf{x} / \hbar \right)} | n_s \rangle \langle n_s | \exp{\left( i \mathbf{p} \cdot \mathbf{x} / \hbar \right)} | \alpha_s \rangle  }{E_0 - E + E(n_s) - E(\alpha_s) + i/2 \Gamma(n_s)} \right| ^2
\end{equation}
...
\label{absorption_general_summed}
	W = \left| M^{\mathrm{nucl}}_{C \to B} \right|^2 \cdot \left| M^{\mathrm{nucl}}_{A \to C} \right|^2 \sum_{\alpha_s} g(\alpha_s) \sum_{n_s} \frac{ \left| \langle n_s | \exp{\left( i \mathbf{p} \cdot \mathbf{x} / \hbar \right)} | \alpha_s \rangle \right|^2  }{\left[ E - E_0 - \left( E(n_s) - E(\alpha_s) \right) \right] + 1/4 \left( \Gamma(n_s) \right)^2 }
\end{equation}
...
This imposes a restriction on the possible values for $\mathbf{k}_{alpha_s}$. 
At the end of a calculation in the finite space, taking the limit $L \to \infty$ should be possible if the calculated observable is independent of the size of the space. When doing the transition to the full space, the sums over $\alpha_s$ and $n_s$ will turn to integrals, because both variables can take on any value.

Using plane waves, the matrix element in the numerator in Eq. \ref{absorption_general_summed} becomes:
...
\end{equation*}
\begin{equation*}
	= N(L)^2 \int \mathrm{d}^3 x \exp{ \left( i \left[ \left( \mathbf{k}_{\alpha_s} + \mathbf{p} \right) - \mathbf{k}_{n_s} \right] \mathbf{x} \right) } \int \mathrm{d}^3 x \exp{ \left( - i \left[ \mathbf{k}_{n_s} - \left( \mathbf{k}_{\alpha_s} + \mathbf{p} \right) \right] \mathbf{x} \right) }
\end{equation*}
\begin{equation*}
...
The symbol $N(L)$ denotes a normalization constant that appears in a finite volume and disappears when going to the continuum. 
It was only included as a reminder.
...
\label{four_momentum_nucleus}
	p^\mu_{\mathrm{X}} = \left( \sqrt{ Au c^2 + k^2 c^2}, \mathbf{k} \right)
\end{equation}
...
\begin{equation}
	...
	p^\mu_{\mathrm{X^*}} = \left( \sqrt{ \left( Au c^2 + E_0 \right)^2 + k'^2 c^2}, \mathbf{k'} \right)
\end{equation}
...
\begin{equation}
	...
	p^0_\mathrm{ph} + p^0_\mathrm{X} = p c + \sqrt{ Au c^2 + k^2 c^2} = \sqrt{ \left( Au c^2 + E_0 \right)^2 + (\mathbf{k} + \mathbf{p})^2 c^2} = p^0_\mathrm{X^*}
\end{equation}
...
\begin{equation}
	\label{recoil_energy}
	E_\mathrm{ph} = \frac{E_0 \cdot Auc^2 + E_0^2/2}{\sqrt{\left( Auc^2\right)^2 + k^2 c^2} - kc}
\end{equation}

The equation is easier to interpret when the nucleus can be considered to be at rest ($k = 0$) before the capture. 
...
\label{absorption_general_plane_wave}
	W = \left| M^{\mathrm{nucl}}_{C \to B} \right|^2 \cdot \left| M^{\mathrm{nucl}}_{A \to C} \right|^2 \int \mathrm{d}^3 k_{\alpha_s}  g(\alpha_s) \int \mathrm{d}^3 k_{n_s} 
\end{equation}
%\begin{equation*}
	%\frac{ \delta \left( left[ \mathbf{k}_{\alpha_s} + \mathbf{p} \right] - \mathbf{k}_{n_s} \right) }{\left[ E - E_0 - \left(E(n_s) - E(\alpha_s) \right) \right] + 1/4 \left( \Gamma(n_s) \right)^2 }
%\end{equation*}
